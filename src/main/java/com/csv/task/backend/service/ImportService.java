...
import com.csv.task.backend.mapper.*;
...
import com.csv.task.backend.repository.*;
...
public class ImportService {
    ...
    public void importServices(byte[] payload) throws IOException {
        ...
        List<com.csv.task.backend.model.Service> servicesToImport = new LinkedList<>();
        List<LoyaltyPointsEntry> newLoyaltyPointEntries = new LinkedList<>();
        for (ServiceDto dto : dtos) {
            Appointment a = appointmentsMap.get(dto.getAppointmentId());
            com.csv.task.backend.model.Service s = serviceMapper.dtoToService(dto, a);
